...
export function baseUiConfig(overrides: Partial<VerbatraConfig> = {}): VerbatraConfig {
  return {
    sourceLocale: "en",
    targetLocales: ["de"],
    format: "i18next-json",
...
export async function makeFixtureProject(
  ...
  const root = await mkdtemp(join(tmpdir(), "verbatra-ui-fixture-"));
  const config = baseUiConfig(overrides);
...
/** A loader over a {@link FixtureProject}, with "override" provenance and no glossary, for injecting into {@link startUiServer}. */
...
export type WithServerOptions = Partial<UiServerOptions>;
