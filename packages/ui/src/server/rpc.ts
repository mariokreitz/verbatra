 ...
 * `status.diff`, and `glossary.get` have real handlers so far; the remaining two are later
 * tickets' handlers.
 *
 ...
 * `status.check` and `status.diff` both do this, and their test files each assert a second call
 * reflects an on-disk edit made between two identical calls. `glossary.get` needs no such test:
 * like `project.snapshot`, it only reads fields already present on the startup-loaded config.
 */
