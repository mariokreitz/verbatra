...
- `src/index.ts` exports `startUiServer`, which binds a server to `127.0.0.1` and serves the SPA
...
at `dist/app` and a fixed token read from the `VERBATRA_UI_DEV_TOKEN` environment variable (with a
