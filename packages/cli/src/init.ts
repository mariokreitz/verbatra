 ...
 * append otherwise. .verbatra-local/ holds process-local, never-committed state (the per-locale
 * write lock files under locks/, and any future run-local scratch data).
 */
function ensureGitignore(cwd: string, streams: Streams): void {
  ...
  if (!existsSync(gitignorePath)) {
    ...
    streams.out("created .gitignore (.env, .env.local, .verbatra-local/)\n");
    return;
  }
  ...
  if (missing.length === 0) {
    streams.out(".gitignore already ignores .env, .env.local, and .verbatra-local/\n");
    return;
  }
