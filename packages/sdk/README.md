...
- `keyIntegrity` reports, per changed key, whether its placeholders and ICU structure still match the source.
- `lockState` reports the lock file's existence, version, and per-locale drift; `loadLockFile` reads the lock file itself.
- `runStatus` reads the persisted review-flag and token-usage snapshot the last `translate` or `watch` run left behind.
- `keyValue` reads one key's current source and target values.
- `editEntry` saves a manually edited translation for one key, and `retranslateEntry` re-runs the provider for one key; both run the same placeholder and ICU integrity checks as a full run and hold the same per-locale write lock.
