...
<p align="center">
  Programmatic API to automate i18n translation and keep your locale files in sync across languages, using OpenAI, Anthropic, Gemini, DeepL, or an openai-compatible local or self-hosted model.
</p>
...
**Not in the latest release yet.** This README documents `main`, and three things on it have not been published to npm: the `doctor()` entry point, the `locales` option on `translate()` and `watch()`, and the `readGlossaryFile` and `updateGlossaryTerm` entry points. Everything else here is in the current release. Check the [`@verbatra/sdk` package on npm](https://www.npmjs.com/package/@verbatra/sdk) for what is installable today.
