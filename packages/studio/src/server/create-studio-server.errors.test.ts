...
import { assertLoopbackAddress, startStudioServer } from "./create-studio-server.js";
...
describe("startStudioServer failure modes", () => {
  let first: StudioServer | undefined;
  let second: StudioServer | undefined;
  ...
  it("surfaces EADDRINUSE as a structured, catchable error and never falls back to another port", async () => {
    first = await startStudioServer({ port: 0, loader: stubLoader() });

    await expect(
      startStudioServer({ port: first.port, loader: stubLoader() }),
...
describe("malformed request handling", () => {
  ...
  it("stays responsive after an authenticated request whose body is aborted mid-upload", async () => {
    assetsRootPath = await mkdtemp(join(tmpdir(), "verbatra-studio-server-"));
