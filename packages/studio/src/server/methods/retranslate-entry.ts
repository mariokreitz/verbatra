 ...
 * pair. Only reachable when `createRpcHandlers` registered it, which requires both the `spend` and
 * `writeToDisk` capabilities. Reads the config resolved once at startup, but re-reads the source,
