...
describe("resolveAssetPath", () => {
  const assetsRootPath = join(tmpdir(), "verbatra-studio-assets-root");
...
describe("readAsset", () => {
  ...
  beforeEach(async () => {
    assetsRootPath = await mkdtemp(join(tmpdir(), "verbatra-studio-assets-"));
  ...
  it("returns undefined for a traversal attempt even when the target exists on disk", async () => {
    const secretPath = join(assetsRootPath, "..", "verbatra-studio-secret-fixture.txt");
    await writeFile(secretPath, "secret");

    try {
      await expect(
        readAsset(assetsRootPath, "/../verbatra-studio-secret-fixture.txt"),
