...
3. The agent confirms a changeset exists for every publishable src change with the
   correct semver bump and a clear summary, that only `@verbatra/sdk`,
   `@verbatra/cli`, and `@verbatra/studio` are published (config, core,
   format-adapters, ai-providers, and exchange stay internal), that OIDC Trusted
   Publishing and provenance and the exact repository.url are intact, and prepares
   the changelog entry.
