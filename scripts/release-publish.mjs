...
//
// Invoked as the changesets/action `publish` input. The child inherits stdio so the action
// still sees `changeset publish`'s own output, which it parses for publishedPackages and
// GitHub release creation; do not capture or rewrite that output here.
//
...
// npm dist-tags are plain identifiers; anything else (especially a leading "-") must never
// reach the publish argv. "next" passes.
const DIST_TAG_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;

/**
 * Decide the explicit dist-tag for `changeset publish`. Pure (no env or I/O access), so it is
 * unit-tested directly; the caller reads .changeset/pre.json and passes its raw content, or
 * null when the file does not exist.
 *
 * Returns the tag to force, or null for a plain untagged publish (no pre mode active). Any
 * state it cannot positively classify is an error: a malformed pre.json must fail the publish
 * loudly, never fall through to an untagged publish that would hit the only-pre fallback.
 *
 * @param {string | null} rawPreJson
 * @returns {string | null}
 */
...
/**
 * Build the pnpm argv for the publish. Pure, unit-tested. `pnpm exec` runs the changeset bin
...
// Only run when invoked as a script, not when imported by the test file for the pure functions.
