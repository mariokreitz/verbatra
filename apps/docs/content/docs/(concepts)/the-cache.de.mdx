...
`verbatra.cache.json` ist eine lokale, regenerierbare Begleitdatei neben deiner Lock-Datei. `verbatra init` trägt sie in `.gitignore` ein, und `translate`, `watch` und `import` ergänzen einen vorhandenen `.gitignore`, dem der Eintrag fehlt, sodass ein mit einer älteren Version angelegtes Projekt ihn beim nächsten Lauf erhält. Halte sie aus der Versionskontrolle heraus: Anders als die Lock-Datei ist sie eine Kostenoptimierung, keine geteilte Grundlinie. Sie lässt nie einen Lauf scheitern. Eine fehlende, beschädigte, zu große oder in der Version unbekannte Datei degradiert zu einem leeren Cache und wird beim nächsten Lauf einfach neu aufgebaut. Um sie von Grund auf neu aufzubauen, lösche die Datei.
