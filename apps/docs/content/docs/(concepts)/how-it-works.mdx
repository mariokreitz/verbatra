...
A run starts from a validated config; see [Config file](/docs/config-file) for where it is found
and what it contains, and for the config verbatra infers when a project has no config file at all. Everything below is the same whether the run came from the CLI, the SDK, or
`watch`.
