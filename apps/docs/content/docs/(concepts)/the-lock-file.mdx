...
Adoption is decided by key presence, never by value, which has three consequences worth knowing
...
- A translation that was already out of date is recorded as current. The lock takes the source hash
  ...
  first run.
- Plural generation can overwrite existing forms. `generatePlurals` (off by default, i18next-JSON
  only) decides what to synthesize from the lock, not from the target file, so on a first run every
  plural form it plans counts as not yet generated and overwrites whatever the target file holds at
  that key. Leave it off while adopting a project that carries hand-written plural forms, and turn
  it on once the baseline exists.
...
writes a locale file, or touches the lock.
