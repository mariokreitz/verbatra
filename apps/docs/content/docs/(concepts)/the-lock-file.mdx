...
Plural generation follows the same adoption rule. A plural form the target file already holds is
kept as it is, never regenerated and never sent to the provider, so `generatePlurals` is safe to
leave on while adopting a project that carries hand-written plural forms; only forms genuinely
absent from the target are synthesized.
