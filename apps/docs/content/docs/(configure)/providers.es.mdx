...
Cada proveedor acota cada solicitud saliente, para que un servidor que acepta la conexión pero nunca responde no pueda atascar una ejecución ni mantener abierto el bloqueo de escritura de un locale. Ese límite es `requestTimeoutMs`, un entero positivo opcional en milisegundos que se acepta en las `options` de los cinco proveedores (`anthropic`, `openai`, `gemini`, `deepl` y `openai-compatible`):
...
DeepL es la única excepción a la cancelación: su cliente no puede abortar una solicitud ya en vuelo, así que al agotarse el tiempo verbatra deja de esperar y sigue, pero la solicitud que ya envió se resuelve en DeepL.
...
| `INVALID_REQUEST` | La solicitud falló la validación antes de cualquier llamada de red: un campo malformado, o un código de locale que DeepL no acepta (un código de origen regional, o un `en` o `pt` escueto como destino). Corrige la configuración o la solicitud; reintentar no ayudará. |
