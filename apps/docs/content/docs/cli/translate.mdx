...
A config file is optional. When the search finds none, verbatra [detects one from the project](/docs/config-file#running-without-a-config-file) and prints what it concluded to stderr before the run, so `translate` needs only a provider API key in the environment. Which provider that is comes from the keys that are set, in the fixed order `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `DEEPL_API_KEY`. With no key set, a detected run fails with `PROVIDER_KEY_MISSING` and exit `2`, except under `--dry-run`, which calls no provider and therefore runs anyway.
