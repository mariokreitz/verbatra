...
A config file is optional. When the search finds none, verbatra [detects one from the project](/docs/config-file#running-without-a-config-file) and prints what it concluded to stderr at startup, so `watch` needs only a provider API key in the environment. With no key set, a detected session fails with `PROVIDER_KEY_MISSING` and exits `2` before the initial run.
