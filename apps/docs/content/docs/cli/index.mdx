...
- `--config <path>` loads that config file instead of searching for one. Every command except `init` takes it; the search order is described in [The config file](/docs/config-file). When the search finds nothing, `translate`, `watch`, `check`, `diff`, `export`, and `import` [detect a config from the project](/docs/config-file#running-without-a-config-file) rather than failing, and print what they detected to stderr; `doctor` and `studio` do not detect.
...
Every command except `init` loads `.env.local` and then `.env` from the working directory before it runs. A variable already set in your real environment always wins. API keys are read only from the environment, never from the config file or a flag: see [Providers](/docs/providers). The read-only commands (`check`, `diff`) and the handoff commands (`export`, `import`) load them too, even though they never call a provider, so the provider a detected project resolves to is the same one `translate` would use.
