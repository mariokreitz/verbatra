...
- A missing **target** locale file is not a problem: [`verbatra translate`](/docs/cli/translate) creates it. A missing **source** locale file is, because every other command fails on it.
