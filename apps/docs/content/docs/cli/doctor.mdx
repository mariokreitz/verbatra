...
`doctor` is the one command that never [detects a config from the project](/docs/config-file#running-without-a-config-file). Its whole job is to validate the configuration you wrote, so validating an inferred one would tell you nothing about your setup: in a project with no config file it reports the configuration check as failed and skips the four checks that need it. Use `verbatra check` or `verbatra translate --dry-run` to see what a detected project would do.
