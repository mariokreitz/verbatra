...
`check` also needs no config file. When the search finds none, verbatra [detects one from the project](/docs/config-file#running-without-a-config-file) and prints what it concluded to stderr before it runs, so a drift gate works on a project that has never been configured. No API key is involved either way. Detection declining is a whole-run failure, exit `2`.
