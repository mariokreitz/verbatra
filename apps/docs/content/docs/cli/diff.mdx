...
`diff` also needs no config file. When the search finds none, verbatra [detects one from the project](/docs/config-file#running-without-a-config-file) and prints what it concluded to stderr before it runs. No API key is involved either way. Detection declining is a whole-run failure, exit `2`.
...
| `2` | could not run: a whole-run error (including a project that could not be detected when there is no config file), or a usage error (including an empty or unknown `--locales` value) |
