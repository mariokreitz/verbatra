...
Eine Konfigurationsdatei ist optional. Findet die Suche keine, [leitet verbatra eine aus dem Projekt ab](/docs/config-file#ohne-konfigurationsdatei-arbeiten) und schreibt vor dem Lauf nach stderr, was sie erkannt hat; `translate` braucht damit nur einen Provider-API-Schlüssel in der Umgebung. Welcher Provider das ist, ergibt sich aus den gesetzten Schlüsseln, in der festen Reihenfolge `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `DEEPL_API_KEY`. Ist kein Schlüssel gesetzt, scheitert ein erkannter Lauf mit `PROVIDER_KEY_MISSING` und Exit `2`, außer unter `--dry-run`, das keinen Provider aufruft und deshalb trotzdem läuft.
