...
- **Una hoja de cálculo puede cambiar lo que abre.** Abrir un `.csv` o `.tsv` en una hoja de cálculo puede convertir valores (`007` pasa a `7`) y puede tratar un valor que empieza por `=`, `+`, `-` o `@` como una fórmula. El entrecomillado no lo evita, porque la hoja de cálculo lo elimina antes de evaluar. verbatra escribe y lee cada valor literalmente a propósito, porque `-5 Grad` y `+49 30 1234567` son traducciones normales. Trata como entrada no confiable cualquier archivo que devuelva un traductor.
