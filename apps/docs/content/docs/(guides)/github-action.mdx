...
<Callout type="warn" title="`@v1` does not resolve yet">
The action has no releases yet, so `mariokreitz/verbatra-action@v1` does not resolve today: a workflow that uses it as-is fails with `Unable to resolve action, unable to find version v1`. Use a full commit SHA from `main`, which is what every example on this page does. `@v1` is documented below so those examples stay correct once the first release is cut.
</Callout>
...
| --- | --- | --- | --- |
| `version` | yes | - | the `@verbatra/cli` version to run. Must be an exact semver version such as `0.7.1` or `0.7.2-next.0`; the step fails immediately on a dist-tag such as `latest`, a range, or a `^`/`~` prefix |
