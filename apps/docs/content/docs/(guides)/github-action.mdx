...
`verbatra/action@v2` is the convenience form and the recommended default: the `v2` tag moves with each v2 release, so fixes reach you without a workflow edit, at the cost of not knowing exactly what runs. SHA pinning is the security-conscious choice either way, and is what the examples on this page use. There is no npm package; the action is consumed only through `uses:`.

`v1` still resolves for workflows that have not upgraded, but it is legacy: it carries no minimum on the `version` input and still symlinks the pinned CLI packages into your repository's `node_modules` on every run, a write that `v2` removes entirely. Point new and existing workflows at `v2`.
...
| --- | --- | --- | --- |
| `version` | yes | - | the `@verbatra/cli` version to run, for example `0.9.3`. Must be an exact semver version; the step fails immediately on a dist-tag such as `latest`, a range, or a `^`/`~` prefix. Under `v2`, must also be `0.9.3` or newer; an older pin fails the step |
| `config-path` | no | `""` | explicit config file to load (maps to `--config`); empty uses the normal config search, which falls back to [project detection](/docs/config-file#running-without-a-config-file) when no config file exists |
