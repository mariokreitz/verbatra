...
The action lives in its own repository, [mariokreitz/verbatra-action](https://github.com/mariokreitz/verbatra-action), separate from the verbatra monorepo. Reference it by repository, pinned to a commit SHA:

```text
uses: mariokreitz/verbatra-action@<commit-sha>
```

`mariokreitz/verbatra-action@v1` is the convenience form: the `v1` tag moves with each v1 release, so fixes reach you without a workflow edit, at the cost of not knowing exactly what runs. SHA pinning is the security-conscious choice and is what the examples on this page use. There is no npm package; the action is consumed only through `uses:`.
...
The action fetches and runs `@verbatra/cli` at exactly the version you pin, so the workflow needs no install step for verbatra itself. `0.6.4` is the release published when this page was last updated; check the [`@verbatra/cli` package on npm](https://www.npmjs.com/package/@verbatra/cli) for the current one and bump the pin deliberately, rather than tracking a moving tag.
...
| --- | --- | --- | --- |
| `version` | yes | - | the `@verbatra/cli` version to run. Must be an exact semver version such as `0.6.4` or `0.6.5-next.0`; the step fails immediately on a dist-tag such as `latest`, a range, or a `^`/`~` prefix |
