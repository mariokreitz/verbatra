...
| --- | --- | --- | --- |
| `version` | yes | - | the `@verbatra/cli` version to run. Must be an exact semver version such as `0.9.0` or `0.9.1-next.0`; the step fails immediately on a dist-tag such as `latest`, a range, or a `^`/`~` prefix |
