...
Todos los comandos excepto `init` también cargan `.env.local` y luego `.env` desde el directorio de trabajo antes de ejecutarse, y las variables de entorno reales siempre ganan, así que en CI normalmente dependerás solo de `env:`.
