...
| Código | Significado |
| --- | --- |
| `0` | éxito: `translate` o `import` terminó bien para cada locale, `check` encontró todos los locales sincronizados, `diff` no encontró cambios pendientes, `export` escribió su libro, `init` preparó el proyecto, `watch` o `studio` se detuvo limpiamente, o se imprimió `--help` o `--version` |
| `1` | la ejecución llegó al final, pero el resultado no está limpio: `translate` o `import` terminó con al menos un locale fallido, `check` encontró un locale desincronizado, `diff` encontró una clave ausente o cambiada, o `studio` falló al apagar su servidor |
