...
Every command except `init` also loads `.env.local` and then `.env` from the working directory before it runs, with real environment variables always winning, so in CI you will normally rely on `env:` alone.
