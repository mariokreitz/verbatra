...
`verbatra/action@v1` ist die bequeme Variante: der `v1`-Tag wandert mit jedem v1-Release mit, Fixes erreichen dich also ohne Änderung am Workflow, dafür weißt du nicht genau, was läuft. Ein SHA-Pin ist ohnehin die sicherheitsbewusste Wahl und wird in den Beispielen auf dieser Seite verwendet. Es gibt kein npm-Paket; die Action wird nur über `uses:` eingebunden.
...
| --- | --- | --- | --- |
| `version` | ja | - | die auszuführende `@verbatra/cli`-Version. Muss eine exakte Semver-Version wie `0.8.0` oder `0.8.1-next.0` sein; der Schritt schlägt sofort fehl bei einem Dist-Tag wie `latest`, einem Bereich oder einem `^`/`~`-Präfix |
