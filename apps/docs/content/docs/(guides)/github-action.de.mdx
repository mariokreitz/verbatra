...
Die Action lebt in einem eigenen Repository, [mariokreitz/verbatra-action](https://github.com/mariokreitz/verbatra-action), getrennt vom verbatra-Monorepo. Referenziere sie per Repository, gepinnt auf einen Commit-SHA:

<Callout type="warn" title="@v1 lässt sich noch nicht auflösen">
Die Action hat noch keine Releases, deshalb lässt sich `mariokreitz/verbatra-action@v1` heute nicht auflösen: ein Workflow, der es unverändert verwendet, scheitert mit `Unable to resolve action, unable to find version v1`. Nimm einen vollständigen Commit-SHA aus `main`, so wie es jedes Beispiel auf dieser Seite tut. `@v1` ist weiter unten dokumentiert, damit diese Beispiele stimmen, sobald das erste Release geschnitten ist.
</Callout>
...
`mariokreitz/verbatra-action@v1` ist die bequeme Variante, sobald es dieses erste Release gibt: der `v1`-Tag wandert mit jedem v1-Release mit, Fixes erreichen dich also ohne Änderung am Workflow, dafür weißt du nicht genau, was läuft. Ein SHA-Pin ist ohnehin die sicherheitsbewusste Wahl und wird in den Beispielen auf dieser Seite verwendet. Es gibt kein npm-Paket; die Action wird nur über `uses:` eingebunden.
...
| --- | --- | --- | --- |
| `version` | ja | - | die auszuführende `@verbatra/cli`-Version. Muss eine exakte Semver-Version wie `0.7.1` oder `0.7.2-next.0` sein; der Schritt schlägt sofort fehl bei einem Dist-Tag wie `latest`, einem Bereich oder einem `^`/`~`-Präfix |
