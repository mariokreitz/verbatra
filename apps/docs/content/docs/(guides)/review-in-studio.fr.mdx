...
Une valeur qui ressemble à une clé d'API fournisseur est masquée avant même d'atteindre le navigateur, et un tel terme ne peut pas être modifié ici, seulement supprimé : ce que le panneau détient est le remplaçant, pas le texte réel. Modifie celui-là directement dans le fichier de glossaire.
