...
- **A spreadsheet may still change what it opens.** Opening a `.csv` or `.tsv` in a spreadsheet can coerce a value (`007` becoming `7`) and can treat a value starting with `=`, `+`, `-`, or `@` as a formula. Quoting does not prevent it, because a spreadsheet strips the quoting before evaluating. verbatra writes and reads every value verbatim by design, since `-5 Grad` and `+49 30 1234567` are ordinary translations. Treat a file coming back from a translator as untrusted input.
