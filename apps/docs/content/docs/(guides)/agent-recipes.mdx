...
Exit `0`. Every per-locale list is present even when it is empty, so an agent can index into it without probing first. A live run adds a `usage` object to each locale and to the summary; a dry run reports none, because it makes no call. [The SDK reference](/docs/sdk) has the full anatomy of a `LocaleSummary`.
...
The first exits `0`, the second exits `1` (the provider endpoint was unreachable), the third exits `0` without spending anything. Step 1 is deliberately `diff` rather than `translate --dry-run`: both are read-only, but `diff` never constructs a provider, so the decision step runs in a job with no secret in it at all.
