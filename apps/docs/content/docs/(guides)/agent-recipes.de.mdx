...
Der erste endet mit `0`, der zweite mit `1` (der Provider-Endpunkt war nicht erreichbar), der dritte mit `0`, ohne etwas auszugeben. Schritt 1 nutzt bewusst `diff` statt `translate --dry-run`: Beide sind schreibgeschützt, aber `diff` baut nie einen Provider auf, der Entscheidungsschritt läuft also in einem Job ganz ohne Secret.
