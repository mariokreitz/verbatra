...
- `concurrency` traduce hasta ese número de locales de destino a la vez (por defecto 1, estrictamente
  ...
  que 1 con `maxTokens` configurado lanza `CONCURRENCY_BUDGET_CONFLICT` (un dry-run queda exento).
- `cache` está activado por defecto y reutiliza la [caché de memoria de traducción](/docs/the-cache)
  local; ponlo en false para omitir la caché en la ejecución (el `--no-cache` de la CLI).
...
- **cacheHits** lista las claves servidas desde la [caché de memoria de traducción](/docs/the-cache)
  ...
  `importWorkbook` (una ejecución de `translate` los deja vacíos): una fila `changed` en blanco, una
  fila del libro que el lector no pudo parsear y una clave duplicada cuya primera aparición ganó.
