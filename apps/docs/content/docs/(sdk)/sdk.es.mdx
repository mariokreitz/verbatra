...
(`verbatra-translations`), ambos exportados como constantes. Los valores aceptados se exportan como `EXCHANGE_FORMATS` y el valor por defecto como `DEFAULT_EXCHANGE_FORMAT`, de modo que una herramienta que envuelve el SDK puede validar un argumento de formato sin fijar la lista en el código. Devuelve `{ path, locales }`: la ruta
