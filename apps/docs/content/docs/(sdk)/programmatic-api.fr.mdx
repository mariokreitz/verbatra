...
Une locale `partial` est une locale dont le fichier a été écrit alors que des clés manquent encore. Ce n'est pas un succès : la CLI sort avec `1` dessus, et ton propre script devrait la traiter pareil, d'où le test de `partial` à côté de `failed` ci-dessus.

Passe `dryRun: true` pour prévisualiser sans appeler de fournisseur ni rien écrire, et `prune: true` ou `generatePlurals: true` pour surcharger ces options de config le temps d'une exécution. Passe `locales: ["de"]` pour restreindre l'exécution à un sous-ensemble des cibles configurées, ce qui te permet de traiter un fournisseur à débit limité une langue à la fois ; une locale non configurée lève `UNKNOWN_LOCALE` avant que quoi que ce soit ne soit lu ou dépensé. Mets `concurrency` au-dessus de 1 pour traduire les locales en parallèle (non autorisé avec un budget `maxTokens` sur une exécution réelle) et `cache: false` pour contourner le [cache de mémoire de traduction](/docs/the-cache).
...
Exporte les chaînes à traduire dans un classeur Excel, confie-le à un traducteur, puis réimporte le fichier rempli. `importWorkbook` exécute les mêmes vérifications de dérive et d'intégrité que `translate` et renvoie la même forme `RunSummary`, donc une passation manuelle s'insère exactement dans le reporting que tu utilises pour une exécution automatisée.
...
## Basculer vers la détection quand il n'y a pas de config

<AvailableFrom version="0.10.0" />

`resolveProjectConfig` est `loadConfigWithMeta` avec un ajout : quand la recherche ne trouve rien, il déduit une config depuis les fichiers de locale du projet et les clés d'API de provider présentes dans l'environnement. Utilise-le dans un outil qui doit fonctionner sur un projet qu'il n'a pas configuré. Un `detection` présent est le signal que rien n'a été trouvé sur disque, et `requireDetectedProvider` est le seul appel à faire avant de dépenser quoi que ce soit.
...
if (resolved.detection !== undefined) {
  const { pattern, format, sourceLocale, targetLocales } = resolved.detection;
  console.error(`detected ${pattern} (${format}): ${sourceLocale} -> ${targetLocales.join(", ")}`);
}

requireDetectedProvider(resolved); // lève PROVIDER_KEY_MISSING sur un projet détecté sans clé
await translate({ config: resolved.config });
```

Pour `check` et `diff`, saute l'appel à `requireDetectedProvider` : ils n'atteignent jamais un provider, donc ils tournent sur un projet détecté sans aucune clé définie. Passe `detect: false` pour conserver l'échec `CONFIG_NOT_FOUND` habituel. `loadConfig` et `loadConfigWithMeta` sont inchangés et lèvent toujours `CONFIG_NOT_FOUND` quand la recherche ne trouve rien. Voir [Fonctionner sans fichier de configuration](/docs/config-file#fonctionner-sans-fichier-de-configuration) pour ce que la détection déduit et ce qu'elle ne déduit pas.
