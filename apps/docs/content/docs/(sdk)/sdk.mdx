...
key from is set, and the source locale file exists. Every check runs even when an earlier one
failed, so one call reports every independent problem. The API key is checked by name only.
