...
CLI sale con `1` ante él, y tu propio script debería tratarlo igual, que es por lo que la
comprobación de arriba prueba `partial` junto a `failed`.

Pasa `dryRun: true` para previsualizar sin llamar a un proveedor ni escribir nada, y `prune: true` o
`generatePlurals: true` para anular esas opciones de configuración durante una ejecución. Pasa
...
## Caer en la detección cuando no hay configuración

<AvailableFrom version="0.10.0" />

`resolveProjectConfig` es `loadConfigWithMeta` con un añadido: cuando la búsqueda no encuentra nada,
deduce una configuración de los archivos de locale del proyecto y de las claves de API de proveedor
del entorno. Úsalo en una herramienta que tiene que funcionar sobre un proyecto que no configuró
ella. Un `detection` presente es la señal de que no se encontró nada en disco, y
`requireDetectedProvider` es la única llamada que hacer antes de gastar nada.
...
requireDetectedProvider(resolved); // lanza PROVIDER_KEY_MISSING en un proyecto detectado sin clave
await translate({ config: resolved.config });
```

Sáltate la llamada a `requireDetectedProvider` para `check` y `diff`: nunca alcanzan un proveedor,
así que se ejecutan sobre un proyecto detectado sin ninguna clave puesta. Pasa `detect: false` para
conservar el fallo `CONFIG_NOT_FOUND` de siempre. `loadConfig` y `loadConfigWithMeta` no cambian y
siguen lanzando `CONFIG_NOT_FOUND` cuando la búsqueda no encuentra nada. Mira
[Ejecutar sin archivo de configuración](/docs/config-file#ejecutar-sin-archivo-de-configuración)
para saber qué deduce y qué no la detección.
