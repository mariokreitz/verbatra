...
Ein als `partial` gemeldetes Locale ist eines, dessen Datei mit weiterhin fehlenden Keys
...
übersteuern. Übergib `locales: ["de"]`, um den Lauf auf eine Teilmenge der konfigurierten Ziele zu
beschränken; so arbeitest du einen ratenbegrenzten Provider Sprache für Sprache ab. Ein nicht
konfiguriertes Locale wirft `UNKNOWN_LOCALE`, bevor irgendetwas gelesen oder ein
kostenpflichtiger Provider-Aufruf gemacht wird. Setze `concurrency` über 1, um Locales parallel zu übersetzen (bei einem Live-Lauf
nicht mit einem `maxTokens`-Budget erlaubt), und `cache: false`, um den
[Translation-Memory-Cache](/docs/the-cache) zu umgehen.
...
Schreiber schicken den Kandidatenwert durch dieselben Integritätsprüfungen wie ein
...
## Auf Erkennung zurückfallen, wenn es keine Konfiguration gibt

<AvailableFrom version="0.10.0" />

`resolveProjectConfig` ist `loadConfigWithMeta` mit einer Ergänzung: Findet die Suche nichts, leitet
es eine Konfiguration aus den Locale-Dateien des Projekts und den Provider-API-Schlüsseln in der
Umgebung ab. Nimm es in einem Werkzeug, das auf einem Projekt arbeiten muss, das es nicht selbst
eingerichtet hat. Ein vorhandenes `detection` ist das Signal, dass auf der Platte nichts gefunden
wurde, und `requireDetectedProvider` ist der eine Aufruf vor allem, was Geld kostet.
...
requireDetectedProvider(resolved); // wirft PROVIDER_KEY_MISSING bei einem erkannten Projekt ohne Schlüssel
await translate({ config: resolved.config });
```

Für `check` und `diff` lässt du den `requireDetectedProvider`-Aufruf weg: Sie erreichen nie einen
Provider und laufen deshalb auf einem erkannten Projekt ganz ohne gesetzten Schlüssel. Mit
`detect: false` behältst du stattdessen den einfachen `CONFIG_NOT_FOUND`-Fehlschlag. `loadConfig`
und `loadConfigWithMeta` sind unverändert und werfen bei einer erfolglosen Suche weiterhin
`CONFIG_NOT_FOUND`. Was die Erkennung ableitet und was nicht, steht in
