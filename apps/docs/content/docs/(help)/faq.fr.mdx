...
## Ai-je besoin d'un fichier de configuration ?

<AvailableFrom version="0.10.0" />

Pas pour démarrer. Sans configuration nulle part, verbatra en déduit une depuis le projet : `check`
et `diff` tournent donc dans un projet jamais configuré, et `translate` n'a besoin que d'une clé
d'API de provider. La détection ne lit que les répertoires que les écosystèmes i18n utilisent par
convention (`locales`, `src/locales`, `public/locales`, `messages`, `i18n`, `src/assets/i18n` et
quelques autres), exige qu'un seul motif de chemin couvre chaque fichier porteur de locale dans
exactement l'un d'eux, résout le format via l'extension ou via la bibliothèque i18n de
`package.json`, et prend l'anglais comme locale source. Chaque exécution écrit d'abord sa
conclusion.

Une configuration que tu as écrite l'emporte toujours, et il t'en faut une pour tout ce que la
détection ne synthétise pas : une locale source non anglaise, plusieurs fichiers d'espace de noms
par locale, un glossaire, un ton, `prune`, `generatePlurals` ou un budget `maxTokens`. La détection
décline plutôt que de deviner : un projet ambigu ou non reconnu est donc une erreur avec un remède,
pas une mauvaise réponse. Voir
[Fonctionner sans fichier de configuration](/docs/config-file#fonctionner-sans-fichier-de-configuration).
...
Voir [Estimer le coût](/docs/estimating-cost) pour la méthode, le cas DeepL et la façon de
...
`GEMINI_API_KEY` ou `DEEPL_API_KEY`, plus la variable que tu nommes pour `openai-compatible`.
Toutes les commandes sauf `init` chargent `.env` et `.env.local` depuis le répertoire de
...
Trois surfaces différentes pour trois tâches différentes. [Configure verbatra avec un agent
...
Aucune des trois ne donne à un agent le moyen de dépenser sans un humain dans la boucle.
`translate` lui-même n'a aucune barrière de dépense propre, donc chaque surface orientée agent
place une étape de confirmation, ou le flag `--allow-spend`, entre l'agent et un véritable appel
au fournisseur.
