...
`translate`, `watch`, `doctor` et `studio` chargent `.env` et `.env.local` depuis le répertoire de
...
Deux choses différentes, traduites de deux façons différentes. Le texte de l'interface (libellés
de navigation, boutons, le pied de page, le texte de la landing page, la FAQ de la landing page)
vit dans `messages/en.json` et ses fichiers frères `de`, `es` et `fr` ; ceux-ci sont traduits
automatiquement avec verbatra lui-même, contre le format `next-intl-json`, via le fournisseur
Gemini, exécuté par `pnpm i18n`. C'est le projet qui utilise ici son propre outil (dogfooding). La
prose de la documentation que tu lis en ce moment, chaque guide et chaque page de référence y
compris celle-ci, est du MDX à suffixe de locale (`page.mdx`, `page.de.mdx`, `page.es.mdx`,
`page.fr.mdx`), maintenu à la main par les mainteneurs du projet : `pnpm i18n` ne traduit que les
fichiers JSON, XLIFF, YAML, ARB et properties, jamais le Markdown, donc il ne touche jamais ces
pages.

Si quelque chose sonne bizarre en allemand, en espagnol ou en français, la bonne marche à suivre
dépend de la moitié concernée : ouvre une issue pour le texte de l'interface, car il est corrigé à
la source anglaise puis retraduit automatiquement ; envoie une pull request pour la prose de la
documentation, car elle est maintenue à la main, et une correction faite par un locuteur natif va
