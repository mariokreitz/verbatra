...
## Brauche ich eine Konfigurationsdatei?

<AvailableFrom version="0.10.0" />

Für den Anfang nicht. Gibt es nirgends eine Konfiguration, leitet verbatra eine aus dem Projekt ab,
`check` und `diff` laufen also in einem Projekt, das nie konfiguriert wurde, und `translate` braucht
nur einen Provider-API-Schlüssel. Die Erkennung liest nur die Verzeichnisse, die die
i18n-Ökosysteme üblicherweise verwenden (`locales`, `src/locales`, `public/locales`, `messages`,
`i18n`, `src/assets/i18n` und eine Handvoll weitere), verlangt, dass ein einziges Pfadmuster jede
Locale-tragende Datei in genau einem davon abdeckt, löst das Format über die Dateiendung oder die
i18n-Bibliothek in der `package.json` auf und nimmt Englisch als Quell-Locale. Jeder Lauf schreibt
vorab, was er erkannt hat.

Eine selbst geschriebene Konfiguration gewinnt weiterhin eindeutig, und du brauchst eine für alles,
was die Erkennung nicht synthetisiert: eine nicht-englische Quell-Locale, mehrere Namespace-Dateien
pro Locale, ein Glossar, einen Ton, `prune`, `generatePlurals` oder ein `maxTokens`-Budget. Die
Erkennung lehnt ab, statt zu raten, ein mehrdeutiges oder unbekanntes Projekt ist also ein Fehler
mit Ausweg, keine falsche Antwort. Siehe
...
`DEEPL_API_KEY`, plus die Variable, die du für `openai-compatible` benennst. Jeder Befehl außer `init` lädt
`.env` und `.env.local` aus dem Arbeitsverzeichnis (echte Umgebungsvariablen gewinnen). Das Konfigurationsschema lehnt unbekannte Keys genau deshalb ab, damit ein Geheimnis
...
lesen: Die Konfiguration lädt und besteht die Validierung, das konfigurierte `format` löst sich zu
einem Datei-Adapter auf, die konfigurierte `provider.id` löst sich zu einer Provider-Factory auf,
die Umgebungsvariable, aus der dieser Provider seinen Schlüssel liest, ist gesetzt (nur dem Namen
nach geprüft, nie am Wert), und die Quell-Locale-Datei existiert, ist eine reguläre Datei und
lässt sich unter diesem Format parsen. Jeder Check läuft auch dann, wenn ein früherer
fehlgeschlagen ist, ein Lauf meldet also jedes unabhängige Problem auf einmal; die Ausnahme ist
die Konfiguration selbst: Schlägt sie beim Laden fehl, werden die vier Checks übersprungen, die
sie brauchen.
...
Drei verschiedene Oberflächen für drei verschiedene Aufgaben. [verbatra mit einem KI-Agenten
...
Keine der drei gibt einem Agenten die Möglichkeit, ohne einen Menschen im Loop Geld auszugeben.
`translate` selbst hat kein eigenes Ausgaben-Gate, jede agentenfähige Oberfläche legt also einen
Bestätigungsschritt oder das `--allow-spend`-Flag zwischen den Agenten und einen echten
Provider-Aufruf.
