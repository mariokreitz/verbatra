...
Zwei verschiedene Dinge, auf zwei verschiedene Arten übersetzt. Der UI-Rahmen
(Navigationsbeschriftungen, Buttons, der Footer, der Landingpage-Text, die Landing-FAQ) lebt in
`messages/en.json` und ihren `de`-, `es`- und `fr`-Geschwistern; die werden mit verbatra selbst
maschinell übersetzt, gegen das Format `next-intl-json`, über den Gemini-Provider, ausgeführt von
`pnpm i18n`. Das Projekt nutzt hier sein eigenes Tool (Dogfooding). Die Doku-Prosa, die du gerade
liest, jeder Guide und jede Referenzseite einschließlich dieser hier, ist locale-suffigiertes MDX
(`page.mdx`, `page.de.mdx`, `page.es.mdx`, `page.fr.mdx`), von Hand gepflegt von den
Projekt-Maintainern: `pnpm i18n` übersetzt nur JSON-, XLIFF-, YAML-, ARB- und Properties-Dateien,
nie Markdown, es rührt diese Seiten also nie an.

Wenn dir im Deutschen, Spanischen oder Französischen etwas seltsam vorkommt, hängt der richtige
Weg davon ab, um welche Hälfte es geht: Öffne ein Issue für UI-Texte, denn die werden an der
englischen Quelle korrigiert und maschinell neu übersetzt; schick einen Pull Request für
Doku-Prosa, denn die ist von Hand gepflegt, und eine Korrektur von einem Muttersprachler landet
direkt in der Datei.
