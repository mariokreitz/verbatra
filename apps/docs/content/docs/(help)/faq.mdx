...
Not to get started. With no config anywhere, verbatra infers one from the project, so `check` and
`diff` run on a project that has never been configured and `translate` needs only a provider API
key. Detection reads only the directories the i18n ecosystems conventionally use (`locales`,
`src/locales`, `public/locales`, `messages`, `i18n`, `src/assets/i18n`, and a handful more),
requires one path pattern to cover every locale-bearing file in exactly one of them, resolves the
format from the file extension or from the i18n library in `package.json`, and uses English as the
source locale. Every run prints what it concluded before it starts.

An authored config still wins outright, and you need one for anything detection does not synthesize:
a non-English source locale, several namespace files per locale, a glossary, a tone, `prune`,
`generatePlurals`, or a `maxTokens` budget. Detection declines rather than guesses, so an ambiguous
or unrecognized project is an error with a remedy, not a wrong answer. See
[Running without a config file](/docs/config-file#running-without-a-config-file).
...
`DEEPL_API_KEY`, plus whatever variable you name for `openai-compatible`. Every command except `init` loads
`.env` and `.env.local` from the working directory (real environment variables win). The config schema rejects unknown keys precisely so a secret cannot end up in a committed
...
Three different surfaces for three different jobs. [Set up verbatra with an AI agent](/docs/start-with-ai)
...
dashboard tab; it is explicitly not for initial project setup.

None of the three gives an agent a way to spend without a human in the loop. `translate` itself has
no spend gate of its own, so every agent-facing surface puts a confirmation step, or the
