...
Two different things, translated two different ways. The UI chrome (navigation labels, buttons,
...
`next-intl-json` format, through the Gemini provider, run by `pnpm i18n`. It is this project
dogfooding its own tool. The documentation prose you are reading right now, every guide and
reference page including this one, is locale-suffixed MDX (`page.mdx`, `page.de.mdx`,
`page.es.mdx`, `page.fr.mdx`) maintained by hand by the project maintainers: `pnpm i18n` only
translates JSON, XLIFF, YAML, ARB, and properties files, never Markdown, so it never touches these
pages.

If something in German, Spanish, or French reads oddly, the fix path depends on which half it is:
open an issue for UI copy, since it gets corrected at the English source and machine-retranslated;
send a pull request for doc prose, since it is hand-maintained and a native-speaker correction goes
