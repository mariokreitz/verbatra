...
`DEEPL_API_KEY`, más la variable que tú nombres para `openai-compatible`. `translate`, `watch` y
`studio` cargan `.env` y `.env.local` desde el directorio de trabajo (las variables del entorno
real ganan). El esquema de configuración rechaza las claves desconocidas precisamente para que un
