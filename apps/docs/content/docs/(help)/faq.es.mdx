...
`DEEPL_API_KEY`, más la variable que tú nombres para `openai-compatible`. `translate`, `watch`,
...
Dos cosas distintas, traducidas de dos maneras distintas. El texto de la interfaz (etiquetas de
...
Gemini, ejecutado por `pnpm i18n`. Es el propio proyecto usando su propia herramienta (dogfooding).
La prosa de la documentación que estás leyendo ahora mismo, cada guía y cada página de referencia
incluida esta, es MDX con sufijo de locale (`page.mdx`, `page.de.mdx`, `page.es.mdx`,
`page.fr.mdx`), mantenido a mano por los mantenedores del proyecto: `pnpm i18n` solo traduce
archivos JSON, XLIFF, YAML, ARB y properties, nunca Markdown, así que nunca toca estas páginas.

Si algo en alemán, español o francés suena raro, el camino correcto depende de a cuál de las dos
mitades pertenece: abre un issue para el texto de la interfaz, porque se corrige en la fuente en
inglés y se retraduce automáticamente; envía un pull request para la prosa de la documentación,
porque se mantiene a mano y una corrección de un hablante nativo va directa al archivo.
