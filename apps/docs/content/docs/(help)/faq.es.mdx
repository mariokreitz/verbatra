...
## ¿Necesito un archivo de configuración?

<AvailableFrom version="0.10.0" />

Para empezar, no. Sin configuración en ninguna parte, verbatra deduce una del proyecto, así que
`check` y `diff` funcionan en un proyecto que nunca se ha configurado y `translate` solo necesita
una clave de API de proveedor. La detección lee únicamente los directorios que los ecosistemas de
i18n usan por convención (`locales`, `src/locales`, `public/locales`, `messages`, `i18n`,
`src/assets/i18n` y algunos más), exige que un único patrón de ruta cubra todos los archivos con
locale de exactamente uno de ellos, resuelve el formato por la extensión o por la biblioteca de
i18n de `package.json`, y usa el inglés como locale de origen. Cada ejecución escribe antes lo que
ha concluido.

Una configuración escrita por ti sigue ganando siempre, y necesitas una para todo lo que la
detección no sintetiza: un locale de origen que no sea el inglés, varios archivos de namespace por
locale, un glosario, un tono, `prune`, `generatePlurals` o un presupuesto `maxTokens`. La detección
declina en lugar de adivinar, así que un proyecto ambiguo o no reconocido es un error con remedio,
no una respuesta equivocada. Mira
[Ejecutar sin archivo de configuración](/docs/config-file#ejecutar-sin-archivo-de-configuración).
...
Consulta [Estimar el coste](/docs/estimating-cost) para el método, el caso de DeepL y cómo
...
`DEEPL_API_KEY`, más la variable que tú nombres para `openai-compatible`. Todos los comandos excepto `init` cargan
`.env` y `.env.local` desde el directorio de trabajo (las variables del entorno real ganan). El esquema de configuración rechaza las claves desconocidas precisamente para que un
...
de API: la configuración carga y pasa la validación, el `format` configurado se resuelve a un
adaptador de archivo, el `provider.id` configurado se resuelve a una factory de proveedor, la
variable de entorno de la que ese proveedor lee su clave está definida (comprobada solo por
nombre, nunca por valor), y el archivo de la locale de origen existe, es un archivo regular y se
parsea bajo ese formato. Cada comprobación se ejecuta incluso si una anterior falló, así que una
sola ejecución informa de todos los problemas independientes a la vez; la excepción es la propia
configuración: si falla al cargar, se omiten las cuatro comprobaciones que la necesitan.
...
Tres superficies distintas para tres trabajos distintos. [Configura verbatra con un agente de
...
autenticada del panel; no está pensado para la configuración inicial.

Ninguna de las tres le da a un agente la manera de gastar sin un humano en el bucle. `translate`
en sí no tiene ninguna barrera de gasto propia, así que cada superficie orientada a agentes coloca
un paso de confirmación, o el flag `--allow-spend`, entre el agente y una llamada real al
proveedor.
