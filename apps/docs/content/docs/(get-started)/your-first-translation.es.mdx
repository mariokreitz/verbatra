...
<Callout title="¿Ya tienes archivos de locale? Este paso es opcional.">
Sin configuración en ninguna parte, verbatra deduce una de tu proyecto: mira en los directorios que los ecosistemas de i18n usan por convención, deduce el patrón de archivos, resuelve el formato por la extensión o por la biblioteca de i18n de tu `package.json`, y toma el inglés como locale de origen. `verbatra check` y `verbatra diff` funcionan entonces ya mismo, y `verbatra translate` solo necesita la clave de API del paso 4. Cada ejecución escribe lo que ha detectado, así que nada se adivina a tus espaldas. Ejecuta `init` igualmente cuando quieras fijar algo de eso, o para poner un glosario, un tono o un presupuesto de tokens, que la detección no aporta. Mira [Ejecutar sin archivo de configuración](/docs/config-file#ejecutar-sin-archivo-de-configuración).
</Callout>
...
Luego abre `.env` y pega tu clave después de `GEMINI_API_KEY=`. Todos los comandos excepto `init` cargan `.env.local` y luego `.env` desde el directorio de trabajo antes de una ejecución, y una variable ya definida en tu shell siempre gana. Consulta [Proveedores](/docs/providers) para ver la variable de cada proveedor.
