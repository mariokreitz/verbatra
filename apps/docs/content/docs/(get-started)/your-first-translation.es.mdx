...
Luego abre `.env` y pega tu clave después de `GEMINI_API_KEY=`. `translate`, `watch` y `studio` cargan `.env.local` y luego `.env` desde el directorio de trabajo antes de una ejecución, y una variable ya definida en tu shell siempre gana. Consulta [Proveedores](/docs/providers) para ver la variable de cada proveedor.
