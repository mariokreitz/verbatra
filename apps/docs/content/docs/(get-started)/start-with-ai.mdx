---
title: Set up verbatra with an AI agent
description: A copy-pasteable prompt for Claude Code, Cursor, or any AI coding agent to add verbatra to a project that already has an i18n setup, gated behind an explicit human confirmation before it ever spends.
---
...
If you are setting verbatra up yourself instead of handing it to an agent, [Your first translation](/docs/your-first-translation) is the human version of the same walkthrough.
...
- [Recipes for agents and scripts](/docs/agent-recipes): the `--json` envelope and exit codes this prompt's own commands rely on, for building your own agent loops on top.
- [Operate Studio with a browser agent](/docs/agent-tools-in-studio): a different surface, for driving an already-set-up project from a browser tab instead of a shell.
- [Your first translation](/docs/your-first-translation): the same setup walkthrough, written for a person instead of an agent.
- [The lock file](/docs/the-lock-file): the full detail behind the adoption-safety behavior this prompt relies on.
