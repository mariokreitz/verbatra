...
Then open `.env` and paste your key after `GEMINI_API_KEY=`. `translate`, `watch`, `doctor`, and `studio` load `.env.local` and then `.env` from the working directory before a run, and a variable already set in your shell always wins. See [Providers](/docs/providers) for every provider's variable.
...
- [Set up verbatra with an AI agent](/docs/start-with-ai): prefer to hand this to an AI coding agent instead? the same walkthrough, packaged as a copy-pasteable prompt.
