...
<Callout title="Already have locale files? This step is optional.">
With no config anywhere, verbatra infers one from your project: it looks in the directories the i18n ecosystems conventionally use, works out the file pattern, resolves the format from the file extension or from the i18n library in your `package.json`, and treats English as the source locale. `verbatra check` and `verbatra diff` then work right now, and `verbatra translate` needs only the API key from step 4. Every run prints what it detected, so nothing is guessed behind your back. Run `init` anyway once you want to pin any of it down, or to set a glossary, a tone, or a token budget, none of which detection supplies. See [Running without a config file](/docs/config-file#running-without-a-config-file).
</Callout>
...
Then open `.env` and paste your key after `GEMINI_API_KEY=`. Every command except `init` loads `.env.local` and then `.env` from the working directory before a run, and a variable already set in your shell always wins. See [Providers](/docs/providers) for every provider's variable.
