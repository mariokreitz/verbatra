---
title: Configura verbatra con un agente de IA
description: "Un prompt listo para copiar y pegar en Claude Code, Cursor o cualquier agente de IA para programar, para añadir verbatra a un proyecto que ya tiene una configuración de i18n, protegido por una confirmación humana explícita antes de que gaste nada."
---
...
Si estás configurando verbatra tú mismo en lugar de dárselo a un agente, [Tu primera traducción](/docs/your-first-translation) es la versión humana del mismo recorrido.
...
verbatra init --provider <anthropic|openai|gemini|deepl> --source <source-locale> \
...
verbatra check --json
verbatra diff --json
verbatra translate --dry-run --json
...
- [Recetas para agentes y scripts](/docs/agent-recipes): el envelope `--json` y los códigos de salida en los que se apoyan los propios comandos de este prompt, para construir tus propios bucles de agente encima.
- [Opera Studio con un agente de navegador](/docs/agent-tools-in-studio): otra superficie distinta, para manejar un proyecto ya configurado desde una pestaña del navegador en lugar de una shell.
- [Tu primera traducción](/docs/your-first-translation): el mismo recorrido de configuración, escrito para una persona en lugar de un agente.
- [El archivo de bloqueo](/docs/the-lock-file): todo el detalle detrás del comportamiento de seguridad de adopción en el que se apoya este prompt.
