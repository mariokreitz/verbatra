...
Öffne dann `.env` und füge deinen Schlüssel nach `GEMINI_API_KEY=` ein. `translate`, `watch`, `doctor` und `studio` laden vor einem Lauf `.env.local` und dann `.env` aus dem Arbeitsverzeichnis, und eine Variable, die schon in deiner Shell gesetzt ist, gewinnt immer. Siehe [Provider](/docs/providers) für die Variable jedes Providers.
...
- [verbatra mit einem KI-Agenten einrichten](/docs/start-with-ai): möchtest du das lieber einem KI-Coding-Agenten überlassen? derselbe Weg, verpackt als copy-pasteable Prompt.
