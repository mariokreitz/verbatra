...
<Callout title="Du hast schon Locale-Dateien? Dann ist dieser Schritt optional.">
Gibt es nirgends eine Konfiguration, leitet verbatra eine aus deinem Projekt ab: Es schaut in den Verzeichnissen nach, die die i18n-Ökosysteme üblicherweise verwenden, ermittelt das Dateimuster, löst das Format über die Dateiendung oder die i18n-Bibliothek in deiner `package.json` auf und nimmt Englisch als Quell-Locale. `verbatra check` und `verbatra diff` funktionieren damit sofort, und `verbatra translate` braucht nur den API-Schlüssel aus Schritt 4. Jeder Lauf schreibt, was er erkannt hat, es passiert also nichts hinter deinem Rücken. Führe `init` trotzdem aus, sobald du etwas davon festschreiben willst, oder um ein Glossar, einen Ton oder ein Token-Budget zu setzen, die die Erkennung alle nicht liefert. Siehe [Ohne Konfigurationsdatei arbeiten](/docs/config-file#ohne-konfigurationsdatei-arbeiten).
</Callout>
...
Öffne dann `.env` und füge deinen Schlüssel nach `GEMINI_API_KEY=` ein. Jeder Befehl außer `init` lädt vor einem Lauf `.env.local` und dann `.env` aus dem Arbeitsverzeichnis, und eine Variable, die schon in deiner Shell gesetzt ist, gewinnt immer. Siehe [Provider](/docs/providers) für die Variable jedes Providers.
